...
class PlansProvider {
 ...
 private function normalize($b):array{if(!is_array($b))return[];if(isset($b['response'])&&is_array($b['response'])){if(isset($b['response']['content']))return$this->normalize($b['response']['content']);return$this->normalize($b['response']);}foreach(['data','registros','planos','items']as$k)if(isset($b[$k]))return$this->normalize($b[$k]);if(array_is_list($b))return array_values(array_filter($b,'is_array'));return isset($b['id_plano'])||isset($b['nomePlano'])?[$b]:[];}
}
